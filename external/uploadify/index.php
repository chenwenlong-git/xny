...
//	{
		...
		else
		{
			...
			if(<?php echo $num ;?> > 1)
			{
				...
				sLi += '<input type="" name="fileurl_tmp[]" value="' + resjson[2] + '">';
			...
			else
			{
				...
				sLi += '<li class="img">';
				sLi += '<img src="../../../' + resjson[2] + '" width="100" height="100" onerror="this.src=\'nopic.png\'">';
				sLi += '<input type="" name="fileurl_tmp[]" id="fileurl_tmp" value="' + resjson[2] + '">';
				sLi += '<a href="javascript:void(0);">删除</a>';
				sLi += '</li>';
...
$(function() {
	...
	$("#SaveBtn").click(function(){
		...
		{
			$(window.parent.document).find("#picurl").val($("input[name^='fileurl_tmp']").val());
		}
...
<div class="W">
	...
	<div class="Wrap">
		...
		<div class="Cont">
			<p class="Note">最多上传<strong>1</strong>个附件,单文件最大<strong>64M</strong>,类型<strong><?php echo GetUpType($type); ?></strong></p>
			<div class="flashWrap">
				<input name="uploadify" id="uploadify" type="file" multiple="true" />
				<span style="display: none"><input type="checkbox" name="iswatermark" id="iswatermark"   /><label>是否添加水印</label></span>
			</div>
			<div class="fileWarp">
				<fieldset>
					<legend>列表</legend>
